	...
	while ($row = db2_fetch_array($stmt)) {
		if (!$row=="") {
			...
			$job = $row[7];
			$salary = $row[11];
			print '<tr><td align=center><a href=employee_update.php?customerNumber=' . $customerNumber . '>' . $customerNumber . '</a><td>'. $customerName.'<td>'. $customerFirst.'<td>'. $workdept.'<td>'. $job.'<td>'. $salary.'</td></tr>';
		}
	}
	/* Close table */
	print '</table><br>';
